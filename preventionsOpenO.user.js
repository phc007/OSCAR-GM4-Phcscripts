// ==UserScript==
// @name     Preventions PHC for OpenO
// @version  1.2
// @grant    none
// @description Adds a menu for selecting immunisations by brand
// @updateURL https://raw.githubusercontent.com/phc007/OSCAR-GM4-Phcscripts/refs/heads/main/preventionsOpenO.user.js
// @downloadURL https://raw.githubusercontent.com/phc007/OSCAR-GM4-Phcscripts/refs/heads/main/preventionsOpenO.user.js
// @include */oscarPrevention/index.jsp*
...
jQuery(document).ready(function() {
    ...
    jQuery("#immunization").autocomplete({  source: tags,  select: function(event, ui) {
		...
		var newWindow = window.open(vPath + '/oscarPrevention/AddPreventionData.jsp?1=1&prevention='+ui.item.value+'&demographic_no='+params.demographic_no, 'AddPreventionWindow', 'width=1000,height=700')
		// After the new window has loaded its content
		jQuery(newWindow.document).ready(function() {
			// Access elements within the new window's document
			jQuery(newWindow.document).find("#din").text(ui.item.din);
			jQuery(newWindow.document).find("tagName[name='dose']").text(ui.item.dose);
			jQuery(newWindow.document).find("#route").val(ui.item.route);
			jQuery(newWindow.document).find("tagName[name='doseUnit']").val(ui.item.units);
			jQuery(newWindow.document).find("tagName[name='manufacture']").val(ui.item.manufacture);
		});
		}
    });
});
